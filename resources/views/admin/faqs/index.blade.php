...
<div class="page-header">
    <div>
        <h1>Faqs</h1>
        <p class="subtitle">Manage faqs</p>
    </div>
...
<div class="card">
    ...
    <form method="POST" action="{{ route('admin.faqs.bulk') }}" id="bulk-form">
        @csrf
        <div class="bulk-bar">
                ...
                @forelse($items as $item)
                    ...
                    <td>@include('admin.components.status-badge', ['status' => $item->is_active ? 'active' : 'inactive'])</td>
                        <td class="actions">
                            ...
                            <a href="{{ route('admin.faqs.show', $item) }}" class="btn btn-sm btn-ghost">View</a>
                            <button form="delete-{{ $item->id }}" class="btn btn-sm btn-danger" onclick="return confirm('Delete this item?')">Delete</button>
                        </td>
                    </tr>
                @empty
                    <tr><td colspan="20">@include('admin.components.empty-state', ['title' => 'No faqs found', 'text' => 'Create your first Faq to get started.'])</td></tr>
                @endforelse
                </tbody>
            </table>
        </div>
    ...
    <form id="delete-{{ $item->id }}" method="POST" action="{{ route('admin.faqs.destroy', $item) }}" class="d-none">@csrf @method('DELETE')</form>
    @endforeach
