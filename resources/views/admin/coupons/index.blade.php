...
<div class="page-header">
    <div>
        <h1>Coupons</h1>
        <p class="subtitle">Manage coupons</p>
    </div>
...
<div class="card">
            ...
            <table class="data-table">
                ...
                @forelse($items as $item)
                    ...
                    <td>{{ $item->starts_at }}</td>
                        <td class="actions">
                            <a href="{{ route('admin.coupons.edit', $item) }}" class="btn btn-sm">Edit</a>
                            <a href="{{ route('admin.coupons.show', $item) }}" class="btn btn-sm btn-ghost">View</a>
                            <button form="delete-{{ $item->id }}" class="btn btn-sm btn-danger" onclick="return confirm('Delete this item?')">Delete</button>
                        </td>
                    </tr>
                @empty
                    <tr><td colspan="20">@include('admin.components.empty-state', ['title' => 'No coupons found', 'text' => 'Create your first Coupon to get started.'])</td></tr>
                @endforelse
                </tbody>
            </table>
        </div>
    ...
    <form id="delete-{{ $item->id }}" method="POST" action="{{ route('admin.coupons.destroy', $item) }}" class="d-none">@csrf @method('DELETE')</form>
    @endforeach
