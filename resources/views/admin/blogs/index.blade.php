...
<div class="page-header">
    <div>
        <h1>Blogs</h1>
        <p class="subtitle">Manage blogs</p>
    </div>
...
<div class="card">
        ...
        <div class="table-responsive">
            <table class="data-table">
                ...
                @forelse($items as $item)
                    ...
                    <td>{{ $item->content }}</td>
                        <td class="actions">
                            <a href="{{ route('admin.blogs.edit', $item) }}" class="btn btn-sm">Edit</a>
                            <a href="{{ route('admin.blogs.show', $item) }}" class="btn btn-sm btn-ghost">View</a>
                            <button form="delete-{{ $item->id }}" class="btn btn-sm btn-danger" onclick="return confirm('Delete this item?')">Delete</button>
                        </td>
                    </tr>
                @empty
                    <tr><td colspan="20">@include('admin.components.empty-state', ['title' => 'No blogs found', 'text' => 'Create your first Blog to get started.'])</td></tr>
                @endforelse
                </tbody>
            </table>
        </div>
    ...
    <form id="delete-{{ $item->id }}" method="POST" action="{{ route('admin.blogs.destroy', $item) }}" class="d-none">@csrf @method('DELETE')</form>
    @endforeach
